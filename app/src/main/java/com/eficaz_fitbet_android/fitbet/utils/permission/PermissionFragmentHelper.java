...
import com.eficaz_fitbet_android.fitbet.utils.permission.callback.OnActivityPermissionCallback;
import com.eficaz_fitbet_android.fitbet.utils.permission.callback.OnPermissionCallback;
...
public class PermissionFragmentHelper implements OnActivityPermissionCallback {
    ...
    public void requestSystemAlertPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            try {
                if (!isSystemAlertGranted()) {
                    Intent intent = new Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS, Uri.parse("package:" + context.getContext()
                            .getPackageName()));
                    context.startActivity(intent);
                } else {
                    permissionCallback.onPermissionPreGranted(Manifest.permission.SYSTEM_ALERT_WINDOW);
